...
bot = telebot.TeleBot('7371855428:AAGcRZqo78naiFjfkhpNg2e7Mw-w8SsKAYw')

# Admin user IDs
admin_id = ["2111548216", "5781146042"]
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 60:
                response = "❄️You Are On Cooldown. Please Wait 1min Before Running The /bgmi Command Again❄️"
                bot.reply_to(message, response)
                return
